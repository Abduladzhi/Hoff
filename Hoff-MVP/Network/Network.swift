...
class  NetworkService {
    ...
    private init() {}
    let urlDomain = "https://hoff.ru/api/v2/get_products_new"
    ...
    public func getTheFood(categoryId: String, sortBy: String, sortType: String, limit: String, offset: String, complition: @escaping(Data?) -> Void) {
        let url = urlDomain + urlCategory + categoryId + urlSortBy + sortBy + urlSortType + sortType + urlLimit + limit + urlOffset + offset + urlDeviceID + urlIsAdroid + urlAppVersion + urlLocation + urlLocation + urlXHoff
