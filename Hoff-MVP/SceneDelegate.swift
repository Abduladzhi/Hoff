...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        let assedlerBuilder = AssedlerScreenBuilder()
        let router = Router(navController: navController, assedlerBuilder: assedlerBuilder)
        let initialVC = router.initialViewCotroller()
        let mainNavVC = UINavigationController(rootViewController: initialVC)
        router.navController = mainNavVC
